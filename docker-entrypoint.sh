...
rm -rf /var/vmail/openemail.local/*
...
echo ${RAND_USER}@openemail.local:{SHA1}$(echo -n ${RAND_PASS} | sha1sum | awk '{print $1}') > /etc/dovecot/dovecot-master.passwd
echo ${RAND_USER}@openemail.local::5000:5000:::: > /etc/dovecot/dovecot-master.userdb
echo ${RAND_USER}@openemail.local:${RAND_PASS} > /etc/sogo/sieve.creds
...
if [ -f ${PUBKEY_MCRYPT} ]; then
  GUID=$(cat <(echo ${openemail_HOSTNAME}) /mail_crypt/ecpubkey.pem | sha256sum | cut -d ' ' -f1 | tr -cd "[a-fA-F0-9.:/] ")
  if [ ${#GUID} -eq 64 ]; then
